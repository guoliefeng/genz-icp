...
genz-icp 本质上是局部 LiDAR 里程计，不是完整 SLAM 或全局地图定位：

- 第一帧 INS 提供绝对坐标起点。（因为考虑要快）
- 后续 INS 默认不参与位姿修正。
...
| 项目 | 实测结果 | 判断 |
| --- | ---: | --- |
| 平均输出频率 | 9.935 Hz | 基本跟上 10 Hz 输入 |
| 最大输出间隔 | 0.319 s | 个别帧存在明显抖动 |
| 平均 CPU | 249.71% | 平均约占用 2.5 核 |
| 峰值 CPU | 780% | 瞬时接近 8 核 |
| 平均 RSS | 约 110.6 MB | 较低 |
| 峰值 RSS | 约 136.8 MB | 可控 |
| 节点磁盘 IO | 接近 0 | 非瓶颈 |

### 2.2 资源结论

1. 主要瓶颈是 CPU，不是内存和磁盘。
2. CPU 峰值主要来自 ICP 对应点搜索、并行线性系统构建、点云转换和局部地图更新。
3. 平均性能满足当前 10 Hz 数据，但 319 ms 的最大间隔意味着系统负载升高时可能丢帧或增加定位延迟。
4. 上述结果不包含 RViz、调试点云、全局地图加载和录包；量产运行应默认关闭这些功能。
